...
Tabla::Tabla(int l, int c, char p[100][100]) {
	this->linii = l;
	this->coloane = c;
	for (int i = 1; i <= linii; i++)
		for (int j = 1; j <= coloane; j++)
...
char Tabla::getPatrat(int linie, int coloana) {
...
void Tabla::setTabla(const char tablaDeJoc[100][100], int l, int c) {
	for (int i = 1; i <= l; i++)
		for (int j = 1; j <= c; j++)
...
void Tabla::setPatrat(int l, int c, const char elem) {
	this->tabla[l][c] = elem;
}
